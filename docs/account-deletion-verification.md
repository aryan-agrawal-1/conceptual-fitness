...
- The internal deletion service schedules permanent erasure exactly 14 days later and does not extend the original deadline when called again.
- The internal service revokes existing app sessions, access tokens, and refresh tokens and disconnects Google Health accounts when scheduling deletion.
- Authentication rejects users already marked for deletion, scheduled sync excludes them, and an in-flight sync cannot reconnect them.
...
Clean `HEAD` has no registered account-deletion route, so users cannot currently schedule deletion through an external API or the iOS app. The committed subset is infrastructure and does not complete issue #13.

Commits: `f35672f`, `b71dc62`, `8357921`, `cc1052c`.

## Verification evidence

- In the integrated working tree, including provisional uncommitted deletion routes and recovery code, 57 focused deletion, authentication, and sync tests passed.
- That same integrated working tree passed the 162-test backend suite with one existing Starlette/httpx deprecation warning. This result is not a clean-`HEAD` endpoint verification.
...
- Working-tree tests cover exact confirmation, wrong-purpose and wrong-device grants, immediate rejection of old access and refresh tokens, ordinary token exchange rejection while deletion is pending, idempotent scheduling, the exact grace-period boundary, provider-revocation failure, cascade erasure, and the sync/deletion race. The route-level test file remains uncommitted with the provisional endpoint.
- No real account was scheduled or deleted; lifecycle checks use isolated fixtures.

## Retention and scope

- The implemented service contract retains active application data during the 14-day grace period solely so deletion can remain recoverable. The app implements no other retention exception.
...
- Commit and register the authenticated scheduling endpoint; no externally available account-deletion API exists in clean `HEAD`.
