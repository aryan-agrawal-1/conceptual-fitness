...
struct DashboardAPIClient {
    ...
    func loadOxygenSaturationDetail(date: Date = Date(), timeframe: ScoreTimeframe) async throws -> OxygenSaturationDetail {
        let dateString = Self.apiDate.string(from: date)
        return try await fetch("/metrics/oxygen_saturation/detail?date=\(dateString)&timeframe=\(timeframe.rawValue)")
    }

    func loadRespiratoryRateDetail(date: Date = Date(), timeframe: ScoreTimeframe) async throws -> RespiratoryRateDetail {
        let dateString = Self.apiDate.string(from: date)
    ...
    func loadStepsDetail(date: Date = Date(), timeframe: ScoreTimeframe) async throws -> StepsDetail {
        let dateString = Self.apiDate.string(from: date)
        return try await fetch("/metrics/steps/detail?date=\(dateString)&timeframe=\(timeframe.rawValue)")
    }

    func loadCaloriesBurnedDetail(date: Date = Date(), timeframe: ScoreTimeframe) async throws -> CaloriesBurnedDetail {
        let dateString = Self.apiDate.string(from: date)
